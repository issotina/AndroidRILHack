...
import io.a41dev.ril2.telephony.cdma.CdmaInboundSmsHandler;
import io.a41dev.ril2.telephony.gsm.GsmInboundSmsHandler;
...
public class SmsBroadcastUndelivered implements Runnable {
    ...
    private void scanRawTable() {
        ...
        try {
            ...
            if (cursor == null) {
                Log.e(TAG, "error getting pending message cursor");
                return;
            }
            ...
            while (cursor.moveToNext()) {
                ...
                try {
                    ...
                    Log.e(TAG, "error loading SmsTracker: " + e);
                    continue;
                }
                ...
                if (tracker.getMessageCount() == 1) {
                    ...
                    if (receivedCount == null) {
                        ...
                        if (newCount == tracker.getMessageCount()) {
                            ...
                            if (DBG) Log.d(TAG, "found complete multi-part message");
                            broadcastSms(tracker);
            ...
            for (SmsReferenceKey message : oldMultiPartMessages) {
                ...
                if (rows == 0) {
                    Log.e(TAG, "No rows were deleted from raw table!");
                } else if (DBG) {
                    Log.d(TAG, "Deleted " + rows + " rows from raw table for incomplete "
                            + message.mMessageCount + " part message");
                }
            }
        } catch (SQLException e) {
            Log.e(TAG, "error reading pending SMS messages", e);
        } finally {
            if (cursor != null) {
                cursor.close();
            }
            if (DBG) Log.d(TAG, "finished scanning raw table in "
