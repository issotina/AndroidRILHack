...
public class UiccCardApplication {
    ...
    void update (IccCardApplicationStatus as, Context c, CommandsInterface ci) {
        synchronized (mLock) {
            ...
            mContext = c;
            mCi = ci;
            IccCardApplicationStatus.AppType oldAppType = mAppType;
            IccCardApplicationStatus.AppState oldAppState = mAppState;
            IccCardApplicationStatus.PersoSubState oldPersoSubState = mPersoSubState;
            mAppType = as.app_type;
            mAppState = as.app_state;
            mPersoSubState = as.perso_substate;
            mAid = as.aid;
            ...
            if (mPersoSubState != oldPersoSubState &&
                    mPersoSubState == IccCardApplicationStatus.PersoSubState.PERSOSUBSTATE_SIM_NETWORK) {
    ...
    private IccRecords createIccRecords(IccCardApplicationStatus.AppType type, Context c, CommandsInterface ci) {
        ...
        } else if (type == IccCardApplicationStatus.AppType.APPTYPE_RUIM || type == IccCardApplicationStatus.AppType.APPTYPE_CSIM){
            return new RuimRecords(this, c, ci);
        } else if (type == IccCardApplicationStatus.AppType.APPTYPE_ISIM) {
    ...
    private void notifyReadyRegistrantsIfNeeded(Registrant r) {
        ...
        if (mAppState == IccCardApplicationStatus.AppState.APPSTATE_READY) {
            if (mPin1State == IccCardStatus.PinState.PINSTATE_ENABLED_NOT_VERIFIED ||
                    mPin1State == IccCardStatus.PinState.PINSTATE_ENABLED_BLOCKED ||
                    mPin1State == IccCardStatus.PinState.PINSTATE_ENABLED_PERM_BLOCKED) {
    ...
    private void notifyPinLockedRegistrantsIfNeeded(Registrant r) {
        ...
        if (mAppState == IccCardApplicationStatus.AppState.APPSTATE_PIN ||
                mAppState == IccCardApplicationStatus.AppState.APPSTATE_PUK) {
            if (mPin1State == IccCardStatus.PinState.PINSTATE_ENABLED_VERIFIED ||
                    mPin1State == IccCardStatus.PinState.PINSTATE_DISABLED) {
    ...
    private void notifyNetworkLockedRegistrantsIfNeeded(Registrant r) {
        ...
        if (mAppState == IccCardApplicationStatus.AppState.APPSTATE_SUBSCRIPTION_PERSO &&
                mPersoSubState == IccCardApplicationStatus.PersoSubState.PERSOSUBSTATE_SIM_NETWORK) {
            if (r == null) {
    ...
    public boolean getIccPin2Blocked() {
        synchronized (mLock) {
            return mPin2State == IccCardStatus.PinState.PINSTATE_ENABLED_BLOCKED;
        }
    }
    ...
    public boolean getIccPuk2Blocked() {
        synchronized (mLock) {
            return mPin2State == IccCardStatus.PinState.PINSTATE_ENABLED_PERM_BLOCKED;
        }
    }
